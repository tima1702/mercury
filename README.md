...
- **react-international-phone** — поле телефона с селектором страны и
  ...
  ТЗ требовало маску `0XXX XXX XXX` (украинский формат), это получится,
  если выбрать UA в селекторе.
...
1. На первом шаге — обязательные поля и валидный телефон. Селектор страны
   меняет маску.
2. На втором — селект «Место работы» подгружается с
   `dummyjson.com/products/category-list`, второй заход на шаг идёт из кеша
   React Query.
3. На любом шаге автосохранение: введите что-то и нажмите «Назад» (на
   шаге 2/3) — данные останутся.
4. Прямой переход на `/step2` или `/step3` без заполнения предыдущих
   шагов редиректит на `/`.
5. На шаге 3 — два ползунка ($200…$1000 шаг 100, 10…30 дней шаг 1).
   После «Подать заявку» — `POST /products/add`, модалка:
